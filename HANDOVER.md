...
### 3a. release-plz crates.io publish — EMAIL VERIFICATION (NEW BLOCKER, 2026-04-26 PM)
**Symptom**: `release-plz` workflow at `855186b` fails with `400 Bad Request — "A verified email address is required to publish crates to crates.io. Visit https://crates.io/settings/profile to set and verify your email address."` — token scopes are now correct (no more 403); crates.io account email is unverified.

**Owner action required (autonomous agent CANNOT fix this)**:
1. https://crates.io/settings/profile → confirm primary email + click verification link in inbox.
2. After verification: `gh workflow run release-plz.yml -R reubenjohn/reposix` and watch.
3. release-plz first run publishes 9 crates in topological order: reposix-core → reposix-cache + reposix-sim → reposix-github + reposix-confluence + reposix-jira → reposix-remote → reposix-cli (`reposix-swarm` is `publish=false`).

**Historical context** (kept for reference if 400 turns back into 403):
- 403 root cause was `CARGO_REGISTRY_TOKEN` generated without `publish-new` scope. Owner regenerated 2026-04-26 with `publish-new` + `publish-update` + `yank` scopes (restricted to `reposix-*`). Token is now correct; only email verification remains.
...
| # | Friction | Personas | P | Fix → verified 2026-04-26 PM |
|---|---|---|---|---|
| 1 | **Headline `92.3%` was wrong; corrected to `89.1%`** across README, docs/index.md, demos/index.md, social/_build_*.py, social/benchmark.svg in this handoff (commit `f6345dd`). MCP comparison fixture is synthesized — not measured against a real MCP server. Add a clearer methodology callout to the vs-MCP page. | mcp-user, skeptical-oss | P0 | **verified-partial — what's left:** README/docs/index.md/social all show `89.1%`; THREE files still leak `92.3%` — `docs/demos/recordings/04-token-economy.transcript.txt` lines 26+61, `docs/demos/recordings/04-token-economy.typescript` lines 27+62. Methodology callout on vs-MCP page still pending. → queued §7-A |
| 2 | **JIRA cache routing bug**: `backend_slug_from_origin(spec.origin)` returned "confluence" for any atlassian.net host. JIRA worktrees pointed at the confluence cache. Fixed in commit (this handoff): now takes the full URL and reads `/jira/` vs `/confluence/` markers. Doctor.rs:271/697 still pass `spec.origin` (display-only impact, not data-corrupting). | code-quality | P0 | **verified-partial — what's left:** doctor.rs:271 + doctor.rs:697 STILL call `backend_slug_from_origin(&spec.origin)` (origin alone, not full URL). Display-only impact confirmed, but the inconsistency with the cli.rs fix means doctor will mis-attribute JIRA worktrees as "confluence" in its findings output. → queued §7-B |
| 3 | **GitHub repo description still says FUSE** (until commit `f6345dd` of this handoff which `gh repo edit`'d it). Verify on hover. | harness-author, skeptical-oss | P0 | **verified-done** — `gh repo view` returns `"REST issue trackers as a git-native partial clone for autonomous coding agents — cat / grep / edit / git push, no MCP schemas, no FUSE."` (literally calls out "no FUSE"). |
| 4 | **`/benchmarks/RESULTS/` 404** on the live site. README + index.md hero now point at the absolute github URL. Verify on next deploy. | mcp-user | P0 | **verified-done** — README.md uses relative `benchmarks/RESULTS.md` (renders correctly on github.com); docs/index.md hero uses absolute `https://github.com/reubenjohn/reposix/blob/main/benchmarks/RESULTS.md`. `curl -sI https://reubenjohn.github.io/reposix/benchmarks/RESULTS/` still 404s but no docs page now links to that 404 path. |
| 5 | **Latency table real-backend cells empty**. Atlassian secrets provisioned in this handoff (commit `ce2f577` time). Re-trigger `bench-latency-v09` to populate confluence column. JIRA still has no creds. | mcp-user, harness-author, skeptical-oss | P0 | **verified-partial — what's left:** `gh secret list` shows ATLASSIAN_API_KEY + ATLASSIAN_EMAIL + REPOSIX_CONFLUENCE_TENANT all set 2026-04-26. JIRA_EMAIL + JIRA_API_TOKEN + REPOSIX_JIRA_INSTANCE NOT in secret list. `bench-latency-cron.yml` reads all of them and would skip JIRA gracefully. `docs/benchmarks/v0.9.0-latency.md` table cells for github/confluence/jira all still empty as of HEAD. Need to dispatch `bench-latency-cron` workflow manually (no run history). → queued §7-C |
| 6 | **Site footer chip says `v0.8.0`** while page text references v0.9.0/v0.11.0. Probably hardcoded in `mkdocs.yml extra` or a theme override. Find and bump. | mcp-user, harness-author | P0 | **verified-NEW — original claim does NOT reproduce.** Live homepage HTML shows `"version": null` in mkdocs material `__config`; no `md-version` chip in markup. `grep -RIn "0\.8\.0" mkdocs.yml docs/` returns ZERO matches. The only stale "v0.8" string is `docs/development/roadmap.md:19` ("current next direction is v0.8") which is unrelated to a footer chip. RESOLVED — no action needed. |
| 7 | **Hero example uses JIRA-style `PROJ-42`** but JIRA backend is read-only; tutorial says `sed + git push` — the example is DOA on JIRA. Either change the headline ID to a generic `0001` (sim-style) or add a connector capability matrix to make capabilities visible. | coding-agent | P0 | **verified-not-started — queued §7-D** — confirmed `docs/index.md` lines 14-30 still use `PROJ-42` in 5 places (curl examples + sed + filename + commit msg). Six-line quickstart at line 68 already uses `sim::demo` + `issues/0001.md`, so headline mismatch persists. |
| 8 | **`git checkout origin/main` vs `git checkout -B main refs/reposix/origin/main` inconsistency**. Fixed in README + docs/index.md in this handoff. Mental-model + concept pages may still have stale form — grep and fix. | coding-agent, harness-author | P0 | **verified-partial — what's left:** README + docs/index.md + docs/tutorials/first-run.md use canonical `git checkout -B main refs/reposix/origin/main`. Stale `git checkout origin/main` form remains in: `docs/concepts/mental-model-in-60-seconds.md:62`, `docs/demos/asciinema-script.md:36+91`. ALLOWED in error-recovery prose (`docs/guides/troubleshooting.md:65+74`, `docs/guides/integrate-with-your-agent.md:83`, `docs/how-it-works/git-layer.md:112`) where it's quoting git's own resolution path after sparse-checkout. → queued §7-E |
| 9 | **GITHUB_TOKEN auto-derived `Debug` leak**. Fixed in commit `ce2f577`. | security-lead | P0 | **verified-done** — `crates/reposix-github/src/lib.rs:119` comment "Hand-rolled `Debug` that REDACTS the `GITHUB_TOKEN`. The auto-derived…" + `impl std::fmt::Debug for GithubReadOnlyBackend` at line 124. |
| 10 | **Phase-51 worktree_helpers re-duplicated** by Phase 55 work — `cache_path_from_worktree` thin-wrappers in gc.rs:166, tokens.rs:69, cost.rs:282 each delegate to canonical but defeat the de-dup intent. Consider inlining or accepting them as thin existence-check wrappers. | code-quality | P1 | **verified-not-started — queued §7-F** — exact line numbers confirmed: gc.rs:166, tokens.rs:69, cost.rs:282 each define a local `fn cache_path_from_worktree` while ALSO importing the canonical one as `resolve_cache_dir` from `crate::worktree_helpers`. |
| 11 | **`Error::Other(String)` 153 occurrences** dominates internal error vocabulary; round-trips JSON through error messages in sim.rs. Replace with typed variants. | code-quality | P1 | **verified-not-started — queued §7-F** — `grep -RIn "Error::Other" crates/ \| wc -l` returns **156** (matrix said 153; grew by 3 since audit). |
| 12 | **Two parallel audit-log schemas** (`audit_events_cache` in cache crate, `audit_events` in sim/confluence/jira) means `reposix doctor` only checks the cache schema. Unify or document. | code-quality, security-lead | P1 | **verified-not-started — queued §7-F** — confirmed: `audit_events_cache` queried from cli/{tokens,cost,history,doctor}.rs; `audit_events` queried from confluence/tests + swarm/{tests,src/main.rs}. Two schemas live in `crates/reposix-core/fixtures/audit.sql` (events) vs `reposix-cache/src/db.rs` (events_cache). doctor.rs:489 only checks `audit_events_cache`. |
| 13 | **No v1.0 stability commitment** anywhere on `/decisions/`. ADR-008 itself documents a breaking URL-shape change in v0.10.0 — bad signal. | harness-author | P1 | **verified-not-started — queued §7-F** — `grep -RIn "v1\.0\|stability\|breaking change\|semver" docs/decisions/` returns ZERO matches. ADR-008 confirms breaking URL-shape change ("closes v0.9.0 Phase 32 carry-forward debt"). |
| 14 | **No documented exit codes / `--json`/`--format=json` output / canonical env-var page / concurrency contract / already-init'd-directory behavior**. Six concrete machine-readability gaps the harness persona surfaced. | harness-author | P1 | **verified-not-started — queued §7-F** — only one mention of "non-zero exit code" in docs/reference/confluence.md:33; no `--json`/`--format=json`/concurrency/already-init'd content found. |
| 15 | **Internal ADRs (002, 003 — deprecated FUSE) still in nav**. `docs/decisions/002-confluence-page-mapping.md` is current (kept ADR); `003-nested-mount-layout.md` references the FUSE-era nested-mount that's gone — mark superseded or delete. | mcp-user, skeptical-oss | P1 | **verified-not-started — queued §7-F** — `mkdocs.yml:145+146` still link both ADR-002 and ADR-003 in nav. ADR-003 frontmatter has `status: accepted` (NOT superseded) and body still says `crates/reposix-fuse/src/{fs.rs,tree.rs,inode.rs}` (a crate that no longer exists). |
| 16 | **`scripts/demos/` (11 files) + `docs/demos/recordings/` (12 typescripts) entirely FUSE-era**. Repo-org audit recommends deletion. ~280 files. | repo-org | P1 | **verified-not-started — queued §7-F** — `ls scripts/demos/` = 12 entries, `ls docs/demos/recordings/` = 12 entries. (Matrix said 11+12; counted 12+12.) The recordings still embed the stale `92.3%` string flagged in row 1. |
| 17 | **`.planning/milestones/v0.{1..8}.0-phases/` is 273 files** (74% of `.planning/` markdown). Condense each into a single `ARCHIVE.md`. | repo-org | P2 | **verified-not-started — queued §7-F** — actual count via `find` = **244** files (matrix said 273; possibly some have been condensed already, or 273 counted v0.9 + v0.10 too). |
| 18 | **`scripts/__pycache__/*.pyc` is committed**. .gitignore miss. | repo-org | P2 | **verified-NEW — original claim does NOT reproduce.** `.gitignore` already contains `__pycache__/` and `*.pyc`. `git ls-files scripts/` returns no pyc/pycache entries. `git ls-files --error-unmatch scripts/__pycache__/` errors with "did not match any file". The .pyc files exist on disk under scripts/__pycache__/ but are NOT tracked. RESOLVED — no action needed. |
| 19 | **Supply chain: no signing, no SBOM, no SLSA, no cosign, no Apple notarization, no Authenticode**. v0.11.0 ships `curl | sh`. | security-lead | P2 | **verified-not-started — queued §7-F** — `grep -RIn "cosign\|SBOM\|SLSA\|notari\|Authenticode\|sigstore" .github/workflows/` returns ZERO matches. |
| 20 | **`research/threat-model-and-critique.md` referenced by CLAUDE.md but missing or unpublished**. | security-lead | P2 | **verified-partial — what's left:** `CLAUDE.md:194` references `research/threat-model-and-critique.md`. The actual file lives at `.planning/research/v0.1-fuse-era/threat-model-and-critique.md` (FUSE-era; pre-pivot). Either move/rewrite for v0.9.0 git-native architecture and publish under `research/`, or update CLAUDE.md ref and delete the v0.1 file. → queued §7-F |
| 21 | **NEW — `docs/development/roadmap.md` still references FUSE-era v0.8 plans + items "deferred to v0.8+: ... FUSE SIGTERM handler"**. roadmap.md:19 says "current next direction is v0.8" and roadmap.md:39 lists FUSE features. Misleads anyone scoping the project's roadmap. | repo-org, mcp-user | P1 | **verified-NEW — queued §7-F** — surfaced while hunting for row 6's `0.8.0` chip. roadmap.md needs full v0.9.0+ rewrite. |
| 22 | **NEW — homepage hero `Comment\nShipped in v0.7.1` is from v0.7 era**. docs/index.md:29 prose example references a shipped-in-v0.7.1 message; combined with PROJ-42 (row 7) and 89.1% (row 1), the hero block is doing time-travel. | mcp-user | P2 | **verified-NEW — queued §7-D** — fold into the row-7 hero rewrite. |
| 23 | **NEW — `bench-latency-cron` workflow has no run history**. `gh run list -w "bench-latency-cron"` returns empty even though secrets are wired. Cron is `0 13 * * 1` (Mondays); Monday 2026-04-21 had no creds, next fire is 2026-04-28. Manual `workflow_dispatch` trigger needed to populate confluence/github cells before the deadline. | mcp-user, harness-author | P0 | **verified-NEW — queued §7-C** — `gh workflow run bench-latency-cron.yml` after pushing this commit. |
