...
| `cadences` | yes | non-empty list; each element one of: `pre-commit`, `pre-push`, `pre-pr`, `weekly`, `pre-release`, `post-release`, `on-demand`, `pre-release-real-backend`, `post-push`. A single gate MAY fire at multiple cadences (e.g., a cheap mechanical check tagged `["pre-commit", "pre-push", "pre-pr"]` runs at every relevant trigger). `post-push` (D-CONV-4) fires at phase/milestone-close AFTER the push lands (orchestrator-side, NOT the pre-push hook) — it is the non-circular slot for a CI-green-on-main check, distinct from `pre-push` which cannot see CI for the commit it is about to push. |
...
| `minted_at` | yes (for rows minted on/after P90 / 2026-07-05) | RFC3339 UTC, **write-once** — set at mint time, NEVER updated by the runner. When present it is the SOLE audit-cutoff anchor (D90-03): a hand-edited `last_verified` can no longer move the `claim_vs_assertion_audit` cutoff (closes the cross-AI H2 backdate hole). Rows whose `last_verified` is ≥ 2026-07-05 but which lack `minted_at` are rejected at load. Legacy rows (no `minted_at`) fall back to the `last_verified` heuristic; null-`last_verified` legacy rows are NOT forced to add it. |
| `coverage_kind` | yes (for transport/perf rows minted ≥ P90) | enum `real-backend` \| `sim-only` \| `mechanical` \| `manual`. Transport/perf-claim rows carrying `minted_at` MUST declare `coverage_kind: real-backend` (or a compliant `waiver` with `until`); **no PASS-with-comment** (F-K4a/D90-05). Enforced at load by `_audit_field.validate_row`. Legacy transport rows (no `minted_at`) are RAISE-only until P95 RBF-D-06 migrates them — never blocked at load. An invalid enum value is rejected for any row. |
| `transport_claim` | optional | tri-state override for transport/perf detection (P90-D1): `true` → force transport (explicit author opt-in, requires `coverage_kind`); `false` → suppress the description-regex (explicit opt-out); absent → regex over `comment` + `id` (NOT `expected.asserts`), gated to the `perf`/`agent-ux`/`security` dimensions. |
| `last_real_grade` / `last_real_verified` | nullable; runner-written | write-history preserved on an env-gated `pre-release-real-backend` skip (RBF-FW-07b / AMENDED D90-04): on a cred-less skip the row flips (and persists) NOT-VERIFIED (fail-closed, OD-2-safe, incl. the P0 litmus), while the prior REAL grade + its timestamp are recorded here (written only when the prior status was PASS/FAIL/PARTIAL; never overwritten by a subsequent skip → idempotent). The skip artifact carries `skip_reason: env-missing` (machine marker) + `skip_detail` (human recovery text) so the churn is explained, not silent. |
...
| `claim_vs_assertion_audit` | yes (for rows minted ≥ 2026-05-08T00:00:00Z) | string ≥50 chars; explains how the verifier's `expected.asserts` would FALSIFY the description claim if the description claim were false. Pre-existing rows with `last_verified` < 2026-05-08 are exempt until P95 RBF-D-06 backfills them. **Null/absent `last_verified` is NOT a blanket exemption** (P109 / D2-RBF-FW11-GRANDFATHER-FIX-01): `catalog_dirty()` drops `last_verified` from unchanged rows, so a legacy row routinely presents null — the grandfather rule therefore keys off **landing-commit presence** (the frozen `GRANDFATHERED_NULL_LV` set in `_audit_field.py`, = rows present at the RBF-FW-11-landing commit `8413184`), NOT the null value itself. A null-`last_verified` row NOT in that set counts as NEW and MUST carry the field (closes the "omit `last_verified` to dodge the requirement" hole). The runner computes sha256 at grade time and persists `claim_vs_assertion_audit_hash` in the artifact for drift detection. **Not enforced against the `docs-alignment` dimension catalog** (`doc-alignment.json`) — that dimension's rows use the distinct per-row schema documented below (`last_verdict`/`last_extracted`, no `last_verified` key at all), so the unified-schema date-cutoff has no anchor field there; RBF-FW-11's runner cross-check skips catalogs whose wrapper `dimension` is `docs-alignment`. |
| `expected.artifact.transcript_path` (or equivalent) | yes if `kind: shell-subprocess` | string path under `quality/reports/transcripts/`; runner refuses to load any kind:shell-subprocess row that lacks an emitting-transcript contract (checked at `expected.artifact.transcript_path`, row-level `transcript_path`, or a "transcript" mention in `expected.asserts` as a transitional fallback). Enforced by `quality/runners/_audit_field.validate_row` at load time. **RUNTIME leg (RBF-FW-08):** at grade time the runner additionally downgrades a would-be shell-subprocess PASS to FAIL unless `artifact.transcript_path` names an existing file containing an `argv:` line — the load-time check only proves the contract was DECLARED, `_audit_field.transcript_evidence_ok` proves a transcript was actually EMITTED. |
