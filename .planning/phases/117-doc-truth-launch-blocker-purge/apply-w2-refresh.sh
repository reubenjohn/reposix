...
$BIN doc-alignment bind --row-id docs/index/latency-24ms-cold-init --claim "278 ms cold init against simulator (CI-canonical)" --source docs/index.md:18-18 --test quality/gates/perf/headline-numbers-cross-check.py::run_cross_check --test quality/gates/perf/test_headline_numbers_cross_check.py::test_hero_surfaces_match_canonical --grade GREEN --rationale "docs/index.md:18 '**278 ms** cold init — simulator, CI-canonical' matches canonical latency.md:40 'reposix init cold' sim=278 ms; run_cross_check LATENCY_CLAIMS init-axis (L181-186) asserts value==canonical and test_hero_surfaces_match_canonical asserts failures==[] (offline). P115 T6 hash refresh."

echo "[2/17 bind] docs/index/latency-8ms-read"
$BIN doc-alignment bind --row-id docs/index/latency-8ms-read --claim "6 ms cached read against simulator" --source docs/index.md:18-18 --test quality/gates/perf/headline-numbers-cross-check.py::run_cross_check --test quality/gates/perf/test_headline_numbers_cross_check.py::test_hero_surfaces_match_canonical --grade GREEN --rationale "docs/index.md:18 '**6 ms** cached read' matches canonical latency.md:42 'Get one record' sim=6 ms; run_cross_check LATENCY_CLAIMS get-axis (L144-149) asserts value==canonical, test_hero_surfaces_match_canonical asserts failures==[]. Hash refresh (same hero line moved 27->278 ms cold-init bytes)."

echo "[3/17 bind] latency-hero-24ms-mismatch"
$BIN doc-alignment bind --row-id latency-hero-24ms-mismatch --claim "Homepage hero: reposix cold init = 278 ms (CI-canonical sim)" --source docs/index.md:18-18 --test quality/gates/perf/headline-numbers-cross-check.py::run_cross_check --test quality/gates/perf/test_headline_numbers_cross_check.py::test_hero_surfaces_match_canonical --grade GREEN --rationale "docs/index.md:18 hero '**278 ms** cold init' matches canonical latency.md:40 'reposix init cold' sim=278 ms; run_cross_check LATENCY_CLAIMS init-axis (L181-186) asserts equality and test_hero_surfaces_match_canonical asserts failures==[]."

echo "[4/17 bind] docs/index/mcp-loop-4883-tokens"
$BIN doc-alignment bind --row-id docs/index/mcp-loop-4883-tokens --claim "MCP tool loop generates ~21k output tokens (live) for the same 3-issue read+edit+push" --source docs/index.md:35-39 --test quality/gates/perf/headline-numbers-cross-check.py::run_cross_check --test quality/gates/perf/test_headline_numbers_cross_check.py::test_hero_surfaces_match_canonical --grade GREEN --rationale "docs/index.md:35 mermaid note '~21k output tokens (live)' derived from token-economy.md:37 GitHub-MCP output-token median 21,171 (round/1000=21); run_cross_check TOKEN_LOOP_CLAIMS output_mcp axis (L216-220) asserts the substring present, test_hero_surfaces_match_canonical asserts failures==[]."
...
$BIN doc-alignment bind --row-id docs/index/reposix-loop-531-tokens --claim "reposix git-native loop uses ~1.2k output tokens (live) for the 3-issue read+edit+push task" --source docs/index.md:31-34 --test quality/gates/perf/headline-numbers-cross-check.py::run_cross_check --test quality/gates/perf/test_headline_numbers_cross_check.py::test_hero_surfaces_match_canonical --grade GREEN --rationale "docs/index.md:31 mermaid note '~1.2k output tokens (live)' derived from token-economy.md:37 reposix output-token median 1,213 (_fmt1/1000='1.2'); run_cross_check TOKEN_LOOP_CLAIMS output_reposix axis (L211-215) asserts the substring present and test_run_cross_check_flags_stale_loop_figure pins the ~1.2k/1,213 pairing."
