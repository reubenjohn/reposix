...
This plan is organized into chapters for navigability:

- **[Objective & Architecture](./01-objective.md)** — Goal, task overview, architectural decisions (mirror_url, Route enum, bus_handler structure).
- **[Task T01 — Catalog-first](./T01-catalog-first.md)** — 6 catalog rows + 6 verifier shells (status FAIL until T06).
- **[Task T02 — bus_url.rs parser](./T02-bus-url-parser.md)** — New parser module + 4 unit tests (Route::Single vs Route::Bus).
- **[Task T03 — SoT-drift wrapper](./T03-sot-drift-wrapper.md)** — `precheck_sot_drift_any` function + 1 unit test.
- **[Task T04 — bus_handler module](./T04-bus-handler.md)** — New handler + main.rs dispatch wiring (State extension, capabilities, export branching).
- **[Task T05 — Integration tests](./T05-integration-tests.md)** — 4 test files (bus_url, capabilities, precheck_a, precheck_b).
- **[Task T06 — Catalog flip & push](./T06-catalog-flip-push.md)** — FAIL → PASS flip + CLAUDE.md update + per-phase push.
- **[References & Threat Model](./08-references-threat-model.md)** — Canonical references, STRIDE threat register, trust boundaries.
- **[Plan-internal close](./09-plan-internal-close.md)** — Phase terminal actions.

## Quick Links

- **Specification:** `.planning/REQUIREMENTS.md` (DVCS-BUS-URL-01..DVCS-BUS-FETCH-01)
- **Architecture:** `.planning/research/v0.13.0-dvcs/architecture-sketch.md` § "3. Bus remote..."
- **Research:** `.planning/phases/82-bus-remote-url-parser/82-RESEARCH.md`
- **Overview:** `.planning/phases/82-bus-remote-url-parser/82-PLAN-OVERVIEW.md`
