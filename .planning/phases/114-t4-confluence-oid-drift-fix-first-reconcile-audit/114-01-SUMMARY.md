...
## Residual gap (honest)
Render parity is achieved for ADF-native pages ONLY. Pre-ADF (storage-only) pages are NOT covered: the fixed LIST path now requests `body-format=atlas_doc_format` exclusively with NO per-page storage fallback, so a pre-ADF page's list render decodes to an empty-or-sentinel body while `get_record` DOES fall back to `?body-format=storage` and returns the real storage HTML — the two renders differ, so `OidDrift` still fires. Whether TokenWorld actually contains such a page is answerable ONLY by the live SC1 gate (Open Question 1, DEFERRED), not by this wiremock wave. If the gate trips `OidDrift` on a DIFFERENT page id post-fix, that page is very likely pre-ADF — file a GOOD-TO-HAVE (follow-up: list-path storage fallback), do not claim universal Confluence compatibility. (This mirrors the honesty correction already applied to `114-RESEARCH.md` OQ1 at `db12187`.)
