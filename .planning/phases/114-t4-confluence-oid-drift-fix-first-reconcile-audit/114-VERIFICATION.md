...
status: human_needed
score: 4/4 artifact-verifiable must-haves verified (SC1+SC2 PENDING-LIVE-RUN, coordinator-run)
overrides_applied: 0
re_verification:
  previous_status: none
  note: initial verification
human_verification:
  - test: "SC1 — `reposix init confluence::TokenWorld /tmp/x && cd /tmp/x && git checkout -B main` against live Confluence TokenWorld (including page 7766017)"
...
**Verified:** 2026-07-15T17:39:22Z
**Status:** human_needed (all artifact-verifiable SCs GREEN; SC1+SC2 real-backend PENDING-LIVE-RUN by coordinator)
...
| --- | --- | --- | --- |
| SC1 | Live TokenWorld `git checkout -B main` (incl. page 7766017) completes with zero oid-drift abort | ⏸ PENDING-LIVE-RUN | Real-backend (coordinator-run). Env-gated; not runnable from here. Pre-ADF residual risk noted (OQ1). |
| SC2 | P0 gate `agent-ux/t4-conflict-rebase-ancestry-real-backend` GREEN against live TokenWorld | ⏸ PENDING-LIVE-RUN | Real-backend (coordinator-run). `quality/gates/agent-ux/t4-conflict-rebase-ancestry-real-backend.sh` exists; needs live creds. |
...
**Score:** 4/4 artifact-verifiable truths (SC3, SC4, and Wave-1/Wave-2 detail truths) verified. SC1 + SC2 deferred to coordinator real-backend run.
...
1. **SC1 — live TokenWorld checkout (incl. page 7766017)**
   - Test: `set -a; . ./.env; set +a; reposix init confluence::TokenWorld /tmp/p114-repro && cd /tmp/p114-repro && git checkout -B main` (leaf isolation — `/tmp`, cd in the SAME invocation).
