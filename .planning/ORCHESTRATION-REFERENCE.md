...
## Handover file template (§3 detail)

Companion to `ORCHESTRATION.md` §3 (the pointer there names this section). The full template
the `relief-handover-writer` agent writes+commits when a coordinator is relieved or the
session pauses:
