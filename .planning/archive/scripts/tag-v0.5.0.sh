#!/usr/bin/env bash
# scripts/tag-v0.5.0.sh — create and push the v0.5.0 annotated tag.
#
# Only run this after:
...
#   4. You've eyeballed the CHANGELOG [v0.5.0] section
#   5. Cargo.toml workspace version has been bumped to 0.5.0
#
...
#   3. v0.5.0 tag must NOT already exist locally
#   4. v0.5.0 tag must NOT already exist on origin
#   5. CHANGELOG.md must contain a `## [v0.5.0]` header
#   6. Cargo.toml workspace version must be 0.5.0
#   7. cargo test --workspace --locked green + scripts/demos/smoke.sh green
#
# Usage:  bash scripts/tag-v0.5.0.sh
#
...
if ! grep -qE '^## \[v0\.5\.0\]' CHANGELOG.md; then
    echo "ERROR: CHANGELOG.md has no '## [v0.5.0]' section" >&2
    exit 1
fi
echo "[guard 5/7] CHANGELOG.md has [v0.5.0] section"
...
#    it does NOT edit.
if ! grep -qE '^version = "0\.5\.0"' Cargo.toml; then
    echo "ERROR: Cargo.toml workspace version is not 0.5.0. Bump it before tagging." >&2
    exit 1
fi
echo "[guard 6/7] Cargo.toml workspace version = 0.5.0"
...
# 8. Extract v0.5.0 body from CHANGELOG for the tag message.
#    Stops at the next `## [` header (which is the previous release) and
#    drops that trailing line via `sed '$d'`.
CHANGELOG_BODY="$(sed -n '/^## \[v0.5.0\]/,/^## \[/p' CHANGELOG.md | sed '$d')"

echo "==> creating annotated tag $TAG"
git tag -a "$TAG" -m "reposix $TAG — synthesized _INDEX.md in FUSE bucket dir (OP-2 partial)
...
echo "and paste the CHANGELOG v0.5.0 section as the body."
