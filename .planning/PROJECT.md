...
- **Why this exists.** From `docs/research/agentic-engineering-reference.md`: MCP burns 100k+ tokens on schema discovery before the first useful operation. POSIX is in the model's pre-training. A git-native read — `cat issues/<id>.md` inside the checkout — measured **~94% fewer output tokens** (1,213 vs 21,171) and **~75% lower cost per session** ($0.21 vs $0.83) than the equivalent GitHub-MCP read. The same P115 live-session benchmark against a real GitHub backend also recorded **~56% smaller total input-context** (244,556 vs 550,219 tokens) — the direct analog to the old "tokens of context" claim — and is offline-reproducible (`docs/benchmarks/token-economy.md`).
