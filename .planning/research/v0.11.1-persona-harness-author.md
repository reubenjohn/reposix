...
**This is the strongest section of the docs.** The trust-model page is unusually honest. Concentric-rings diagram, explicit lethal-trifecta framing (Simon Willison citation), `Tainted<T>` newtype with a trybuild compile-fail test, append-only SQLite triggers (`BEFORE UPDATE/DELETE RAISE`), single egress chokepoint (`reposix_core::http::client()` with `clippy::disallowed_methods` enforcing no direct `reqwest::Client::new()`).

**Talking points I could give my users with a straight face:**
- "Outbound HTTP is allowlisted — `REPOSIX_ALLOWED_ORIGINS` defaults to loopback only; you must opt into each backend explicitly."
- "Server-controlled fields (`id`, `created_at`, `version`, `updated_at`) are stripped on push so a poisoned issue body cannot rewrite metadata."
- "Every network operation appends a row to an append-only SQLite audit table with documented `op` vocabulary."
- "Push-time conflict detection rejects stale-base writes with the standard git `fetch first` error."

**Caveats I'd have to own:**
- The trust-model page's `What's NOT mitigated` section is forthright: *"Shell access bypasses every cut. ... reposix is a substrate for safer agent loops — it is not a sandbox."* That's the right disclosure but I'd have to re-state it for my harness's threat model.
- *"The simulator is itself attacker-influenced"* and *"confused-deputy across backends"* are real and mine to communicate.
- Cache compromise (full file swap) defeats append-only triggers. I'd have to be clear: if the host is owned, reposix's audit table is not your last line of defense.

This part of the docs is the reason I'd consider an opt-in recipe rather than just declining outright.

---

## The "integrate with your agent" guide — completeness audit

**It's a pointer page, not a recipe.** Quote from the page: *"Full vetted recipes (Claude Code, Cursor, Aider, Continue, Devin, SWE-agent CI fixtures) ship in v0.12.0; this is the pointer page."* So the page knows it isn't done.

**What it does well:**
- Three named patterns (Claude Code skill / Cursor shell loop / custom SDK loop) each with the correct architectural advice ("don't register reposix as a tool with the model — the substrate is the point").
- "Gotchas" section is concrete: don't truncate stderr, don't add `reposix list` to the allow-list, watch for `egress_denied` audit rows.
- Honest "What integration is NOT" section disambiguates from MCP servers, custom CLIs, sandboxes.

**What's missing for me to ship:**
- **No detection sequence** for "did `reposix init` succeed vs partially fail?" The page does not tell me how to verify post-init that the working tree is healthy. (Answer is presumably `reposix doctor`, but the integrate page doesn't link to it.)
- **No per-pattern code that actually runs.** The custom-SDK-loop section is pseudo-code (`init := subprocess.run(["reposix", "init", spec, path])`), not a compilable example.
- **No teardown story.** When my user uninstalls my harness, what state does reposix leave behind? Where's the cache? How big does it get? (Addressed elsewhere — `reposix gc` is in troubleshooting — but not linked from the integrate guide.)
- **No version-pinning advice.** Should my harness pin `reposix >= X.Y, < Z`? Use `cargo binstall`? Vendor the binary? Ship the installer? No guidance.
- **No "how do I tell my users 'reposix is installed' from inside my harness UI?"** `which reposix` works; not mentioned.
