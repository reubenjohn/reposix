# v0.12.0 — Autonomous Execution Protocol
...
> **Future home.** Most of this document becomes `quality/PROTOCOL.md` in P57 (per QG-03). It lives here in `.planning/research/` for now because the framework doesn't exist yet — the first phase agent reads this from research, then ships it as PROTOCOL.md as part of P57.
...
```
1. Read .planning/STATE.md (current cursor)
2. Read .planning/ROADMAP.md ## v0.12.0 → find this phase's entry
3. Read .planning/REQUIREMENTS.md ## v0.12.0 → find requirements mapped to this phase
4. Read this file (PROTOCOL.md once it lands)
5. Read quality/SURPRISES.md (if exists) — what dead ends has earlier work hit?
6. Read previous phase's verdict: cat quality/reports/verdicts/p<N-1>/<latest>.md
```
...
Agent({
  ...
  prompt: `
    You have ZERO session context. You will not be persuaded by narrative.
    Read .planning/REQUIREMENTS.md ## v0.12.0 → identify the requirements mapped to Phase <N>.
    Read quality/catalogs/*.json → find the catalog rows tagged with this phase.
    For each row:
      1. Re-run the verifier.script with the documented args
      2. Compare actual artifact contents against expected.asserts
      3. Grade PASS / FAIL / PARTIAL
    Read CLAUDE.md → confirm this phase's contributions are documented per QG-07.
    Read quality/SURPRISES.md → confirm any pivots are journalled.
    Output: scored table with citations to file:line for every claim.
    Refuse to grade GREEN unless every catalog row PASSes AND CLAUDE.md is updated.
  `
})
```
...
| Phase close verification | `general-purpose` with the prompt template in Step 7 |
...
| `quality/PROTOCOL.md` | ≤500 lines. New rules go in here only when they apply to MORE than one dimension; dimension-specific rules go in `quality/gates/<dim>/README.md`. |
...
- `v0.12.0-vision-and-mental-model.md` — WHY this protocol exists
- `v0.12.0-naming-and-architecture.md` — `quality/` directory + catalog schema
- `v0.12.0-roadmap-and-rationale.md` — phase-specific application of this protocol
- `v0.12.0-decisions-log.md` — the owner conversation that produced these rules
