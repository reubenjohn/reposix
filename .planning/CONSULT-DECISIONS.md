...
## 2026-07-16 [SELF] DP-3 inversion — mint the 3 uncatalogued hero-number rows via `bind`, NOT the backfill fan-out

- **Context:** Manager intake (SURPRISES-INTAKE 2026-07-16, filed at `029bde7`) + #47→#48
  handover §6 step 3a: bind the 3 uncatalogued hero-number surfaces (`docs/index.md:17`,
  `README.md:27`, `docs/concepts/reposix-vs-mcp-and-sdks.md:29-31`) presenting the new
  ~94.3%/~74.9% figures to doc-alignment rows. Manager specified the refresh flow first,
  named the backfill flow as the fallback mint path.
- **Empirical finding (OP-1, not a code reading):** `reposix-quality doc-alignment
  plan-refresh <doc>` returns `{"stale_rows": []}` for all 3 docs — refresh re-grades only
  EXISTING drifted rows, structurally cannot mint net-new/unbound surfaces. Confirmed the
  manager's flagged risk with real output.
- **DP-3 3-question test (reader-digester evidence, file:line):** (1) PROBLEM = 3 hero
  surfaces present the new figures with no catalog binding; ungated once the 11-row retire
  batch lands. (2) SIMPLEST MECHANISM = `bind` (one additive call per surface) pointing at
  tests that ALREADY assert the exact substrings — `headline-numbers-cross-check.py::run_cross_check`
  (TOKEN_CLAIMS :227-234 checks each surface's literal substring) + `bench_token_economy.py::_assert_headline_reduction`
  (:89 `EXPECTED_OUTPUT_REDUCTION_PCT=94.3`, ±1.0% band from committed captures). (3) The
  sketched backfill flow adds a 25-35-subagent DISCOVERY fan-out (`plan-backfill`→`merge-shards`,
  re-extracts the WHOLE catalog) where discovery is already done (surfaces + test targets
  named) — and would risk churning the 11 pending-RETIRE_PROPOSED rows the owner holds
  confirm-retire commands for. → INVERT to `bind`.
- **Decision:** mint the 3 net-new rows via `bind` (surgical, additive, touches only the 3
  new rows; leaves the 11 retire-pending rows untouched). Not E2 — catalog schema / tests /
  dimension all unchanged; mint-path choice only. Manager delegated the fallback mint-path
  choice ("try refresh first" + named a general "flow").
- **Reversibility:** fully reversible (`git revert` the bind commits; or `propose-retire`
  the new rows). Committed per-doc for clean relief points.
- **Status:** OPEN — executor dispatched to mint + `walk` + commit per-doc this rotation;
  DELETE this entry once the 3 BOUND rows land + `walk` is GREEN.
