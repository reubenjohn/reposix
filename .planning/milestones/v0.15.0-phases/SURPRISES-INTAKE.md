...
- [`surprises-intake/part-08.md`](surprises-intake/part-08.md) — 6 entries:
  ...
  - 2026-07-19 | discovered-by: P126 close-bookkeeping lane | severity: HIGH | **RESOLVED (P127 T1, DP-2)** — root cause was an ownership-BLIND `sweep_7878()` port kill; ownership-scoped fix `413886e1` on RED repro `b3b1b407`, fresh code-review PASS (mechanism-not-symptom, gate GREEN 3/3, F-K4b congruent), regression locked by gate `docs-repro/sweep-7878-ownership-scoped`
  - 2026-07-19 | discovered-by: P126 gsd-verifier (WARN-1) | severity: MEDIUM | PARTIAL — structure/hermetic-test-network-isolation lying local PASS mint vs ~0.02s CI-sandbox fast-fail: honesty half CLOSED (P127 T2 re-mint WAIVED `f8b8d9ed`), CI-portability CODE fix kept-filed for P128
  - 2026-07-19 | discovered-by: P127 T4 (traceability reconcile, gsd-executor) | severity: MEDIUM | PARTIAL — REQUIREMENTS DRAIN rows stale vs closed Phases 119/124; T4 flipped unentangled DRAIN-14/25, HELD 11/13/22/23/24 pending open intake items (SIGKILL #1 / F-K4b-container / provenance / ORCHESTRATION.md re-grown >ceiling)
- [`surprises-intake/part-09.md`](surprises-intake/part-09.md) — 5 entries (P127 T1 noticing):
  - 2026-07-19 | discovered-by: P127 T1 lane (gsd-executor) | severity: MEDIUM | OPEN — (a) ORCHESTRATION.md re-grown to 24119B > 20000B ceiling (Phase-119 split didn't stick); DRAIN-11's concrete false-Complete; re-split before 2026-08-08 waiver lapse; P128
  - 2026-07-19 | discovered-by: P127 T1 lane (gsd-executor) | severity: MEDIUM | OPEN — (b) `zero-shot-onboarding.sh:55` ownership-blind `pkill -f "reposix-sim.*${SIM_BIND}"` fallback (same class as T1); setsid + PGID group-kill, drop pkill; P128
  - 2026-07-19 | discovered-by: P127 T1 lane (gsd-executor) | severity: LOW | OPEN — (c) pid-recycle residual in the new `sweep-7878-ownership-scoped.sh` gate: `SWEEP_OWNED_*` identities linger to the final EXIT sweep (strictly narrower than the T1 bug); drop each after its kill; P128
  - 2026-07-19 | discovered-by: P127 T1 lane (gsd-executor) | severity: LOW | OPEN — (d) `container-rehearse-sigkill-safe.sh` is 16972B = 170% of the 10000B `.sh` ceiling (waived to 2026-08-08); factor helpers into `lib/`; P128
  - 2026-07-19 | discovered-by: P127 T1 lane (gsd-executor) | severity: LOW | OPEN — (e) ownership-blind `kill $(lsof -ti:7878)` operator-recovery hints in `container-rehearse.sh:223` + `lib/sim-lifecycle.sh:61,64`; narrow to `-sTCP:LISTEN`; P128
