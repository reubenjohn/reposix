...
- **Source:** NOTICED during P117-03 W2 (SC5a benchmarks/README.md de-fabrication) · **Severity: LOW** · STATUS: OPEN — tag perf-gate hygiene / dead-code.
...
- **Source:** GUARD-tripped during the P117-02 regression fix (the W2 troubleshooting.md re-attach recovery block pushed the fenced-block count to 51) · **Severity: MEDIUM** · STATUS: OPEN — tag docs-repro / gate-design. **BLOCKS the docs-repro/snippet-coverage pre-push gate on main until resolved** (uncovered_count=0, so coverage is fine; only the count ceiling is red).
