...
- [`good-to-haves/part-10.md`](good-to-haves/part-10.md) — 7 entries:
  ...
  - From Phase 125 W1 plan 125-01 (SC3/DRAIN-12 mirror-drift hint fix, 2026-07-18) — GTH-V15-90 (two `structure/file-size-limits` residual files grew further over ceiling)
  - From Phase 125 close (SC2 self-heal test-coverage gap, gsd-verifier VERDICT #2, 2026-07-19) — GTH-V15-91 (local git-fixture harness for the litmus self-heal reconcile-ordering, runnable without a real backend)
  - From owner-directed session-analyzer expansion via /gsd-capture (global skill, 2026-07-19) — GTH-V15-92 (milestone-close session-retrospective ceremony step using the expanded session-analyzer U6)
  - From /gsd-quick CI-infra fix (pre-pr timeout band-aid, 2026-07-19) — GTH-V15-93 (improve CI rust-cache hit rate so pre-pr cold builds don't approach the timeout wall — the real fix behind the 15→28min band-aid)
  - From Phase 126 close + wave noticings (docs-alignment tooling polish, 2026-07-19) — GTH-V15-94 (split the 84,994-char doc_alignment.rs), GTH-V15-95 (skip RETIRE_CONFIRMED rows in the out-of-eligible warning emitter → 0 warnings), GTH-V15-96 (DRY the two parallel eligible/backfill file lists), GTH-V15-97 (split run.py + test_run.py over the .py ceiling), GTH-V15-98 (OWNER-ACTION: canary-probe HARD-FAIL in the user-global doc-clarity-review SKILL.md)
- [`good-to-haves/part-11.md`](good-to-haves/part-11.md) — 2 entries:
  - From Phase 126 close + verifier grade (docs-alignment tooling polish, 2026-07-19) — GTH-V15-99 (decouple the F-K4b grade-time demote from implicit minted_at activation → explicit opt-in flag), GTH-V15-100 (reconsider cadence membership of the 2 live-network badge P2s — ~127s serial pre-pr cost → weekly + on-demand)
