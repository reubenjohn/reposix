...
**Goal:** Close the remaining 9 `MISSING_TEST` rows + 1 prose-fix in `docs/social/linkedin.md`. Four narrative rows propose-retire as untestable design-framing; four UX claims bind to hash-shape verifiers (P71 `--test <path>` pattern); `spaces-01` binds to a `reposix spaces --help` smoke test (subcommand verified live during prep); polish2-06-landing binds to a doc-grep asserting the connector capability matrix exists in `docs/index.md`. Linkedin FUSE drift gets fixed via prose edit (no catalog change — the BOUND row at line 21 stays BOUND; walker re-hashes after refresh).
