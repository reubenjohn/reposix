...
> **Status:** scoped 2026-05-08. Phases P97–P106 derive from `.planning/research/v0.13.2-cross-link-fidelity/` (10-chapter design folder, 28 ADRs after the 2026-05-08 design-scrutiny session, 4 examples, 3 cold-reader passes, 2 design-scrutiny subagent passes). Owner ratifications captured in `.planning/research/v0.13.2-cross-link-fidelity/08-open-questions.md` § "Owner ratification": Q2 = Rust sub-command of `reposix-quality`; Q6 = cred-hygiene regex pre-commit only; Q14 = 10-phase decomposition. Two follow-on subagent reviews surfaced ADRs 23–28: tag-filter (framework-orchestration decoupling), per-scope `max_l3_per_push` + derived discovery, catalog/tracker file split, L3 dispatcher reuse via `persist_artifact`, shared markdown walker, shared `heading_subtree_hash`. Phase numbering picks up after v0.13.0's extended absorption phases P89–P96 (per `.planning/research/v0.13.1-real-backend-frictions/READY-TO-EXECUTE.md`).
