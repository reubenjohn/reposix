...
This ledger approached the milestone-hygiene 44000 B ceiling (`quality/gates/agent-ux/p111-milestone-hygiene.sh` assert E) and was split into per-part child files under `surprises-intake/` via `scripts/split_ledger.py` (byte-exact round-trip verified). Every entry is preserved verbatim; append new entries to the last part (or a new part) and add the title here. **Cross-part recount (2026-07-13, updated by the red-main window-aware-gate hotfix):** of 35 v0.14.0 entries across all 4 parts, 21 are terminal (RESOLVED / DEFERRED) and **14 are OPEN** — 8 in `part-03.md` (lines 82, 134, 149, 164, 186, 198, 210, 241) and 6 in `part-04.md` (4 filed 2026-07-13 by the tag-prep mechanical + 2 filed 2026-07-13 by the red-main hotfix; see the bullets below). `part-04.md` also carries a 5-item "carried forward, still-unfiled" one-liner cluster (LOW, not counted in the 35 — they are pointers, one of which references an existing part-03.md entry rather than duplicating it). Re-derive this count from the parts' `STATUS:` fields, not from memory, whenever entries are added.
...
- [`surprises-intake/part-03.md`](surprises-intake/part-03.md) — 10 entries:
  ...
  - 2026-07-12 21:15 | discovered-by: B1 (litmus self-heal proof, verify-against-reality) | severity: HIGH | RESOLVED-in-item-4b (commit d1cc811 — fail-closed ADF translation, non-empty sentinel + export refuse; no empty-body PATCH escapes)
  ...
  - 2026-07-13 | discovered-by: items 4a/4b code review (marker-spoof taint surface) | severity: LOW | OPEN (attacker-authored body containing the ADF sentinel line is unpushable — fail-closed write-DoS, escape hatch = strip marker; v0.15.0 collision-resistant sentinel)
- [`surprises-intake/part-04.md`](surprises-intake/part-04.md) — 6 entries + a 5-item carried-forward one-liner cluster:
  - 2026-07-13 | discovered-by: successor #13 (v0.14.0 tag-prep mechanical) | severity: HIGH | OPEN (9th-probe cadence cannot reach honest exit-0 on a sub-2.34 VM; cross-ref Ruling #4)
  - 2026-07-13 | discovered-by: successor #13 (v0.14.0 tag-prep mechanical) | severity: HIGH | OPEN (transient verification-JSON clobber footgun bit hard this session, twice, incl. an orphaned background shell; RATIFICATION.md had to route around it)
  - 2026-07-13 | discovered-by: successor #13 (v0.14.0 tag-prep mechanical) | severity: MEDIUM | OPEN (orphaned-background-shell dispatch hazard — long real-backend runs must be foreground, never background-and-exit)
  - 2026-07-13 | discovered-by: successor #13 (v0.14.0 tag-prep mechanical) | severity: MEDIUM | OPEN (run.py has no .env autoload; naive dispatch silently env-gate-skips every real-backend row)
  - 2026-07-13 | discovered-by: red-main hotfix (brew-formula window-aware gate) | severity: MEDIUM | OPEN (`test_freshness_synth` named "hermetic" is not hermetic — drives the LIVE network-touching weekly runner; flaked RED on this very release-window drift; + ensure_ascii re-encode dirties subjective-rubrics.json under full-suite runs)
  - 2026-07-13 | discovered-by: red-main hotfix (brew-formula window-aware gate) | severity: LOW-MEDIUM | OPEN (systemic: release-plz bump-merge publishes to crates.io BEFORE the v-tag; window re-opens EVERY release; gate-tolerance half RESOLVED in this hotfix, pipeline race routed to v0.15.0)
  - + carried-forward one-liners (LOW; dead `PROTECTED_IDS` var, agent_flow_real.rs/translate.rs split candidates, stale part-03.md:59-61 STATUS text, run_helper_export_real stderr discard [already filed part-03.md, not duplicated], oversized CONSULT-DECISIONS.md)
