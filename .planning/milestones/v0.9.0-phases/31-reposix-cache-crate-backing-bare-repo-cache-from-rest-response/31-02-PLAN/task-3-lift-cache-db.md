...
# Task 3: Lift cache_db.rs

Move `cache_db.rs` from `reposix-cli` to `reposix-cache::cli_compat` for schema unification.

## Files

- `crates/reposix-cli/Cargo.toml`
- `crates/reposix-cli/src/cache_db.rs` (to be deleted)
- `crates/reposix-cli/src/lib.rs`
- `crates/reposix-cli/src/main.rs`
- `crates/reposix-cli/src/refresh.rs`
- `crates/reposix-cache/src/cli_compat.rs` (new)
- `crates/reposix-cache/src/lib.rs`

## Read first

- `crates/reposix-cli/src/cache_db.rs` — code to be lifted.
- `crates/reposix-cli/src/lib.rs`, `main.rs`, `refresh.rs` — refactor call sites.
- `crates/reposix-cli/Cargo.toml` — add dep.
- `crates/reposix-cache/src/lib.rs` — module home for cli_compat.

## Behavior

The `cache_db.rs` file from v0.8.0 defines the CLI's metadata database (separate from the new v0.9.0 `cache_schema.sql`). Lifting it into `reposix-cache` gives the single-row `refresh_meta` schema one home instead of drifting across two crates during v0.9.0. The public API is unchanged:
...
Existing call sites in `reposix-cli/src/refresh.rs` continue to work via a re-export shim. The lift is verbatim — no schema changes, no behavior changes.

## Action

**Step 1 — Create `crates/reposix-cache/src/cli_compat.rs` by copying `crates/reposix-cli/src/cache_db.rs` VERBATIM.**

Update the file header doc comment to say:
...
Place it alphabetically (likely after `pub mod cache;`).

**Step 3 — Add `reposix-cache` as a dep to `crates/reposix-cli/Cargo.toml`.**

Locate the `[dependencies]` table and add (alphabetically if possible):

```toml
reposix-cache = { path = "../reposix-cache" }
```

**Step 4 — Delete `crates/reposix-cli/src/cache_db.rs` entirely.**

Use `rm` or `git rm`.

**Step 5 — Update the two files that module-declare or use `cache_db`:**

**5a.** In `crates/reposix-cli/src/lib.rs`, find the `pub mod cache_db;` (or `mod cache_db;`) declaration and REPLACE with a re-export shim:
...
**5b.** In `crates/reposix-cli/src/main.rs`, if there is a `mod cache_db;` line, DELETE it (main binaries pull the re-export through lib.rs if the crate has one, or directly `use reposix_cache::cli_compat as cache_db;` if not).
...
Tests moved with the code and now live at `crates/reposix-cache/src/cli_compat.rs` as `#[cfg(test)] mod tests { ... }` inside the module.
