...
> **Honest coverage note (added P91, RBF-A-06):** the four rows above shipped [x] at P79 against the **simulator only** — real-backend `attach` dispatch didn't exist yet. P91 (`91-attach-sync-real-backend-wiring`) wired real-backend dispatch via `backend_dispatch` (D91-03; tests `attach_real_*`/`sync_real_*` in `agent_flow_real.rs`), fixed the `ForkAsNew`/`Abort` stubs (D91-04), added `remote.pushDefault` + missing-helper warnings, and proved the flow against real TokenWorld via the milestone-close vision litmus (D91-06/D91-07, 11/11 asserts). Bucket-aware paths (`issues/`/`pages/`, D91-13) also landed here.
...
**Coverage note (added P91):** the `P79 (sim) / P91 (real-backend)` split above is the ONLY dual-phase entry in this table — see the honest-coverage note under `reposix attach` core.
