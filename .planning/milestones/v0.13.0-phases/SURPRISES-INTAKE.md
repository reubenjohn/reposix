...
**What:** A dark-factory-style fresh-eyes T2 audit of the round-tripper flow (vanilla clone → `reposix attach` → edit → `git push` → `reposix doctor`) against real TokenWorld surfaced 4 HIGH frictions. All 5 goal steps ultimately completed, but each finding is a trap a first-time agent falls into:
...
**Correction to this entry's original "cannot be verified locally" claim:** it CAN — `file://` transport drives the same `git upload-pack` + allowFilter on git 2.25. New `crates/reposix-cache/tests/partial_clone_serves.rs` reproduces the exact "repository corruption" death (RED) and proves the filtered clone + checkout serves real blob content (GREEN); plus reposix-remote unit tests for `parse_want_oid` and the teaching-hint detection. The coverage gap that hid this (no test ever drove a SUCCESSFUL fetch) is now closed at the cargo level; `agent-ux/real-git-push-e2e` remains the box-independent git≥2.34 CI backstop. Final PASS proof: CI (see phase-close report).
