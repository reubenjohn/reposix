...
The reject message reads its own cache's ref state and translates the staleness window into a human sentence. The recovery is the same `git pull --rebase && git push` you already know, and — as of the v0.14.0 RBF-LR-03 fix (Phase 105) — it reconciles regardless of *how* the SoT moved: a peer git-side push, an **external REST write** (web UI / direct PATCH), and an SoT **deletion** all recover through the one documented command. Both formerly-breaking drift sources (external-REST and deletion) used to abort the rebase with a low-level git internal error because the cache rebuilt a non-descendant "Sync from REST snapshot" commit; the fix chains the fetch fast-import onto the tracking tip inside a private import ref namespace (the new tip now contains the old one) and emits a full-rebuild `deleteall` so deletions propagate instead of resurrecting. Regression-guarded by `agent-ux/rebase-recovery-reconciles`. See [troubleshooting — Bus-remote `fetch first` rejection](../guides/troubleshooting.md#bus-remote-fetch-first-rejection).
