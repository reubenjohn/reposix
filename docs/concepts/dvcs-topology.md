...
The reject message reads its own cache's ref state and translates the staleness window into a human sentence. The recovery is the same `git pull --rebase` you already know — **with one caveat.** When the SoT moved via a *git-side push*, `reposix sync --reconcile && git pull --rebase` recovers cleanly. When it moved via an **external REST write** (web UI / direct PATCH), the cache rebuilds a non-descendant "Sync from REST snapshot" commit and `git pull --rebase` aborts with a low-level git internal error — the RBF-LR-03 deep-reconciliation limitation (§Cache-coherence, "a same-second write racing the cursor makes it silently wrong"), fixed in the v0.14.0 reconciliation redesign. Workaround until then: re-clone with a fresh `reposix init` and re-apply unpushed local commits. See [troubleshooting — Bus-remote `fetch first` rejection](../guides/troubleshooting.md#bus-remote-fetch-first-rejection).
