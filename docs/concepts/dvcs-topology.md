...
The reject message reads its own cache's ref state and translates the staleness window into a human sentence. The recovery is the same `git pull --rebase && git push` you already know, and — as of the v0.14.0 RBF-LR-03 fix (Phase 105) — it reconciles regardless of *how* the SoT moved: a peer git-side push, an **external REST write** (web UI / direct PATCH), and an SoT **deletion** all recover through the one documented command. Both formerly-breaking drift sources (external-REST and deletion) used to abort the rebase with a low-level git internal error because the cache rebuilt a "Sync from REST snapshot" commit that was not a descendant of your tracking tip; the fix rebuilds that snapshot *as* a descendant of your current tip inside a private ref namespace (so the new tip now contains the old one, and the rebase fast-forwards) and does a full rebuild so a deleted record actually disappears instead of resurrecting. Regression-guarded by `agent-ux/rebase-recovery-reconciles`. For a Pattern-C **attach** tree specifically, this same recovery works because `reposix attach` seeds `refs/reposix/origin/main` at the mirror merge-base at attach time — giving the fetched SoT snapshot a common ancestor with your local commit-before-attach edits, so `git pull --rebase && git push` replays them cleanly instead of hitting a cross-root `add/add` wall. See [troubleshooting — Bus-remote `fetch first` rejection](../guides/troubleshooting.md#bus-remote-fetch-first-rejection).
