...
| Header | Purpose |
|--------|---------|
| `X-Reposix-Agent` | Logical agent identity. Required for audit attribution. `git-remote-reposix` sets this to `<binary-name>-{pid}`. The simulator uses it as the rate-limit bucket key (spoofing is a known gap; see [trust model](../how-it-works/trust-model.md)). |
