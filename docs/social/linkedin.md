...
🚀 reposix — a working FUSE filesystem + git-remote-helper for issue trackers. In a simulated benchmark (a representative 35-tool Jira-shaped MCP catalog vs a reposix shell session for the same task) it used 📉 ~92% fewer tokens. Real-world numbers are still TBD — the simulator isn't the real GitHub API yet — but the direction is clear and the fixture is auditable in the repo.
