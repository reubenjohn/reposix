...
A complete forensic query (e.g., "which JIRA write came from which `git push`?") joins both: the helper-level `helper_push_accepted` row lives in `audit_events_cache`; the per-issue `update_record` rows live in `audit_events`. The two DBs deliberately differ in journal mode: `cache.db` (`reposix-cache::open_cache_db`) sets [SQLite WAL](../reference/glossary.md#sqlite-wal) ([write-ahead logging](https://sqlite.org/wal.html), where writes go to a separate file and merge into the main DB on checkpoint, so readers don't block writers) for concurrent-reader friendliness, while the per-backend audit DB (`reposix-core::open_audit_db`) keeps SQLite's default rollback journal — an **intentional asymmetry** (deliberate by outcome): the audit DB is single-writer-per-process and append-only, so WAL's readers-don't-block-writers benefit doesn't apply the same way there, and the simpler journal keeps the forensic file self-contained. Both DBs are append-only at the SQL level regardless of journal mode: `BEFORE UPDATE` and `BEFORE DELETE` triggers raise rather than allow row mutations. The split keeps `reposix-cache` strictly cache-side and lets backend adapters fail closed without coupling to the cache's SQLite connection lifecycle (POLISH2-22 friction row 12; physical unification deferred to v0.12.0+).
