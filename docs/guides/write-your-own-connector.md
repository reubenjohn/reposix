...
- `async fn list_records(&self, project: &str) -> Result<Vec<Record>>` — full project listing. Empty project returns an empty vec, NOT an error. Paired with a completeness-aware sibling `async fn list_records_complete(&self, project) -> Result<Listing>` (where `Listing { records, is_complete }`): the default impl calls `list_records` and reports `is_complete = true`, but backends that page against a remote and can truncate (GitHub, JIRA, Confluence) MUST override it to report `is_complete = false` at every truncation exit — the cache reads `is_complete` before pruning its `oid_map` and SKIPS the deletion-direction prune on a truncated listing, so records beyond the page cap are never mistaken for deletions.
