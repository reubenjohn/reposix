...
- **FUSE write-path rewire onto `IssueBackend`.** Phase 10 rewired the read path; the write path still speaks the simulator's REST shape directly via `crates/reposix-fuse/src/fetch.rs`. Lifts cleanly once the write-side backend methods above land. v0.4+.
- **`git-remote-reposix` backend-abstraction.** The remote helper still hardcodes the simulator. The `IssueBackend` trait makes the rewire mechanical; tracked for v0.4+.
