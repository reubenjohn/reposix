...
fn gix_082_exposes_expected_surface() {
    ...
    let hash_only = gix::objs::compute_hash(
        repo.object_hash(),
        gix::object::Kind::Blob,
        b"hello",
    )
    .expect("compute_hash");
    assert_eq!(hash_only, oid, "compute_hash must agree with write_blob OID");
    ...
    assert_eq!(head.target().try_id().map(gix::ObjectId::from), Some(head.id().detach()));
}
