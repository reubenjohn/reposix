...
//!
//! The CLI spawns `reposix-sim` as a child process. We want to resolve it
//! relative to `current_exe()` (i.e. the `target/{debug,release}/` sibling
//! of `reposix`) rather than via `$PATH`, so a malicious binary earlier on
//! `$PATH` cannot spoof the child (T-03-08 spoofing mitigation).
//!
...
//! This is slower on the first invocation but keeps the dev loop
...
pub fn resolve_bin(name: &str) -> Command {
    ...
    // Fallback: cargo run. The binary name is also the crate package
    // name, so we pass `name` through directly.
