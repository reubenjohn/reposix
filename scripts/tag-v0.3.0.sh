...
echo "[guard 2/6] working tree clean"
...
echo "[guard 3/6] no local tag $TAG"
...
echo "[guard 4/6] no remote tag $TAG on origin"
...
echo "[guard 5/6] CHANGELOG.md has [v0.3.0] section"

# 6. All tests green (belt-and-suspenders; fast-fail before tagging)
echo "[guard 6/6] verifying workspace is green..."
cargo test --workspace --locked
bash scripts/demos/smoke.sh
echo "[guard 6/6] cargo test + smoke.sh green"
