#!/usr/bin/env bash
# scripts/tag-v0.4.1.sh — create and push the v0.4.1 annotated tag.
#
# Only run this after:
...
#   4. You've eyeballed the CHANGELOG [v0.4.1] section
#   5. Cargo.toml workspace version has been bumped to 0.4.1
#
...
#   3. v0.4.1 tag must NOT already exist locally
#   4. v0.4.1 tag must NOT already exist on origin
#   5. CHANGELOG.md must contain a `## [v0.4.1]` header
#   6. Cargo.toml workspace version must be 0.4.1
#   7. cargo test --workspace --locked green + scripts/demos/smoke.sh green
#
# Usage:  bash scripts/tag-v0.4.1.sh
#
...
if ! grep -qE '^## \[v0\.4\.1\]' CHANGELOG.md; then
    echo "ERROR: CHANGELOG.md has no '## [v0.4.1]' section" >&2
    exit 1
fi
echo "[guard 5/7] CHANGELOG.md has [v0.4.1] section"
...
#    it does NOT edit.
if ! grep -qE '^version = "0\.4\.1"' Cargo.toml; then
    echo "ERROR: Cargo.toml workspace version is not 0.4.1. Bump it before tagging." >&2
    exit 1
fi
echo "[guard 6/7] Cargo.toml workspace version = 0.4.1"
...
# 8. Extract v0.4.1 body from CHANGELOG for the tag message.
#    Stops at the next `## [` header (which is the previous release) and
#    drops that trailing line via `sed '$d'`.
CHANGELOG_BODY="$(sed -n '/^## \[v0.4.1\]/,/^## \[/p' CHANGELOG.md | sed '$d')"

echo "==> creating annotated tag $TAG"
git tag -a "$TAG" -m "reposix $TAG — decouple sim REST shape from FUSE write path + git-remote helper
...
echo "and paste the CHANGELOG v0.4.1 section as the body."
